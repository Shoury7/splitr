...
- Gets all 1-to-1 expenses (i.e., expenses without groupId).

- Computes how much the user owes and how much they are owed.

- Applies settlements (payments already made to settle debts).

- Returns structured data for showing in the UI.
*/
...
// 1. Gets current user's _id.
// 2. Calculates timestamp for Jan 1 of the current year.
// 3. Fetches all expenses from that date onward.
// 4. Filters expenses where the user is either payer or part of splits.
// 5. Adds only the user's split.amount to totalSpent (ignores what they paid for others).
// 6. Returns the total personal share the user spent in the year.
...
// 1. Gets current user's _id.
// 2. Gets all expenses from Jan 1 using "by_date" index.
// 3. Filters expenses involving the user.
// 4. Initializes monthly totals to 0 for all 12 months.
// 5. For each expense, adds only user's split.amount to the respective month.
// 6. Returns sorted array of { monthTimestamp, totalSpent } per month.
